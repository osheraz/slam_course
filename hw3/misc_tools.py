...
def plot_state(particles, landmarks, gt,avg_list_x,avg_list_y,best_list_x,best_list_y):
    ...
    plt.plot(avg_list_x, avg_list_y, 'k-', label='avg')

    # estimated traveled path of average
    plt.plot(best_list_x, best_list_y, 'p--', label='best-at-each-itr', linewidth=1, markersize=1)

    # gt traveled path of best particle
    plt.plot(np.array(gt)[:,0], np.array(gt)[:,1], 'b-' , label='gt')
